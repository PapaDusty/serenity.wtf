...
local Library do 
    ...
    -- Fixed image downloading logic
    for _, ImageData in pairs(Library.Images) do
        local FilePath = Library.Folders.Assets .. "/" .. ImageData[1]
        if not isfile(FilePath) then
            local Success, ImageContent = pcall(game.HttpGet, game, ImageData[2])
            if Success and ImageContent then
    ...
    local Instances = { } do
        ...
        function Instances.Create(self, Class, Properties)
            ...
            for Property, Value in pairs(NewItem.Properties) do
                if NewItem.Instance[Property] ~= nil then
        ...
        function Instances.AddToTheme(self, Properties)
        ...
        function Instances.ChangeItemTheme(self, Properties)
            if not self.Instance then 
                return
            end
        ...
        function Instances.MakeDraggable(self)
            if not self.Instance then 
                return
            end
        ...
        function Instances.MakeResizeable(self, Minimum, Maximum)
            if not self.Instance then 
                return
            end
        ...
        function Instances.OnHover(self, Function)
            if not self.Instance then 
                return
            end
        ...
        function Instances.OnHoverLeave(self, Function)
            if not self.Instance then 
                return
            end
        ...
        function Instances.Tooltip(self, Text)
            if not self.Instance then 
                return
            end
    ...
    function Library.GetImage(self, Image)
    ...
    function Library.DeleteConfig(self, Config)
    ...
    function Library.SaveConfig(self, Config)
        if isfile(Library.Folders.Configs .. "/" .. Config) then
    ...
    function Library.RefreshConfigsList(self, Element)
        local CurrentList = { }
        local List = { }
    ...
    function Library.GetTheme(self)
        local Theme = { } 

        local Success, Result = Library:SafeCall(function()
            for Index, Value in pairs(Library.Flags) do 
                if type(Value) == "table" and Value.Color and StringFind(Value.Flag or "", "Theme") then
    ...
    function Library.LoadTheme(self, Theme)
    ...
    function Library.DeleteTheme(self, Theme)
    ...
    function Library.SaveTheme(self, Theme)
        if isfile(Library.Folders.Themes .. "/" .. Theme) then
    ...
    function Library.RefreshThemeList(self, Element)
        local CurrentList = { }
        local List = { }

        local ThemeFolderName = StringGSub(Library.Folders.Themes, Library.Folders.Directory .. "/", "")

        for Index, Value in pairs(listfiles(Library.Folders.Themes)) do
    ...
    function Library.ToRich(self, Text, Color)
        return `<font color="rgb({MathFloor(Color.R * 255)}, {MathFloor(Color.G * 255)}, {MathFloor(Color.B * 255)})">{Text}</font>`
    end

    function Library.ChangeItemTheme(self, Item, Properties)
        Item = Item.Instance or Item
    ...
    function Library.ChangeTheme(self, Theme, Color)
    ...
    function Library.Watermark(self, Text, Icon)
        local Watermark = { }
    ...
    function Library.KeybindList(self)
        local KeybindList = { }
    ...
    function Library.Notification(self, Text, Duration, Color, Icon)
        local Items = { } do
    ...
    local Components = { } do
        function Components.Window(Data)
            local Items = { } do 
        ...
        function Components.Page(Data)
            ...
            Items["Inactive"]:Connect("MouseButton1Down", function()
                for _, Value in pairs(Data.PagesTable) do
        ...
        function Components.Section(Data)
            local Items = { } do 
    ...
    function Components.Slider(Data)
        ...
        Items["RealSlider"]:Connect("InputBegan", function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then 
                ...
                if typeof(Value) ~= "number" or Value ~= Value then -- checks for NaN
    ...
    function Components.Dropdown(Data)
        ...
        function Dropdown:SetOpen(Bool)
            ...
            if Dropdown.IsOpen then 
                ...
                for _, Value in pairs(Items["OptionHolder"].Instance:GetDescendants()) do 
    ...
    function Components.Listbox(Data)
        local Listbox = { 
            Value = { },
            Flag = Data.Flag,
            Options = { }
    ...
    function Library.Window(self, Data)
        Data = Data or { }
        ...
        local Items = Components.Window({
            ...
            if Window.GradientTitle and Window.GradientTitle.Enabled then
                local UIGradient = Instances:Create("UIGradient", {
                    ...
                    Rotation = 0,
                    Color = RGBSequence{
                        RGBSequenceKeypoint(0, Window.GradientTitle.Start or FromRGB(255, 0, 0)),
                        RGBSequenceKeypoint(0.25, Window.GradientTitle.Middle or FromRGB(0, 255, 0)),
                        RGBSequenceKeypoint(1, Window.GradientTitle.End or FromRGB(0, 0, 255))
                    }
                })
                ...
                Library:Connect(RunService.Heartbeat, function()
                    local GradientOffset = MathAbs(MathSin(tick() * (Window.GradientTitle.Speed or 1)))
    ...
    function Library.Pages.PlayerList(self, Data)
        local Playerlist = {
            Window = self.Window,
            Page = self,
    ...
    function Library.Sections.Button(self, Data)
        ...
        function Button:SubButton(Data)
            ...
            local NewSubbutton = NewButton:SubButton(SubButton)
    ...
    function Library.Sections.Label(self, Text, Alignment)
        local Label = {
            Window = self.Window,
            Page = self.Page,
            Section = self,
